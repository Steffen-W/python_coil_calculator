...
def find_Archimedean_spiral_length(n : int, a : float)  -> float:
    ...
    phi = 2 * n * M_PI
    l = (a / (4 * M_PI)) * (phi * math.sqrt(1 + phi * phi) + math.log(phi + math.sqrt(1 + phi * phi)))
    return l
...
def Mut(r1 : float, r2 : float, x : float, g : float) -> float:
    ...
    Ec = EF(c)
    result = -0.004 * M_PI * math.sqrt(r1 * r2) * ((c - 2 / c) * Ec.Fk + (2 / c) * Ec.Ek)
    return result

def SelfInductanceStraightWire(l : float, dw : float) -> float:
    r = 0.5 * dw * math.exp(0.25)
    result = 0.002 * (l * math.log((l + math.sqrt(l * l + r * r)) / r) - math.sqrt(l * l + r * r) + l / 4 + r)
    return result
...
def HeliCoilS(Lw : float, psi : float, r : float, dw : float, w : float, t : float, isRoundWire : bool, accuracy : int) -> float:
    ...
    if (psi == 0):
        ...
        ThetaO = -math.fabs(math.atan(math.sqrt(1 - cosThetaO * cosThetaO) / cosThetaO))
        Y0 = 0
    else :
        ...
        t0 = t1
        t1 = t0 - (k1 + math.cos(t0) - k2 * t0 * t0) / (-math.sin(t0) - 2 * k2 * t0)
...
def getMultiLayerN(I : float, D : float, dw : float, k : float, lk : float, gap : float, Ng : int, result : _CoilResult):
    ...
    while (Ltotal < I): #  start calculation loop increasing N-turns to reach requiring inductance (I)
        ...
        M = 0 # start calculation loop of the mutual inductance - current turn (N) + all another turns (j)
        if (N > 1):
    ...
    while (Ltotal < Ind):
        ...
        if (n > 1):
            for j in range(n, 1, -1):
                ...
                D = math.sqrt(math.pow(nx - jx, 2) + math.pow(ny - jy, 2)) # distance to in-phase straight conductor of previous turn
                ...
                Db = math.sqrt(math.pow(nx - jx, 2) + math.pow(ny + jy + a0, 2))
                # distance to opposite cunductor between the current turn and previous (side b)
                Da = math.sqrt(math.pow(nx - jx, 2) + math.pow(ny + jy + b0, 2))
    ...
    while (Ltotal <= I):
        ...
        if (N > 1):
            for j in np.arange(N,1,-2):
                jy = r0 + k * (j - 2)
                r = ny - jy
                gmd = math.exp(((r * r) / (w * w)) * math.log(r) + 0.5 * (1 - ((r * r) / (w * w))) * math.log(w * w + r * r) + (2 * r / w) * math.atan(w / r) - 1.5)
                gmr = math.sqrt(ny*jy)
...
def rectPCBSpiral(N : int, a : float, b : float, s : float, w : float, h : float) -> float:
    ...
    try:
        ...
        AMD_b /= N * N
        La = 0.2 * (a * math.log(a + math.sqrt(a * a + AMSD_L))- a * log_GMD_L - math.sqrt(a * a + AMSD_L) + AMD_L)
        Lb = 0.2 * (b * math.log(b + math.sqrt(b * b + AMSD_L))- b * log_GMD_L - math.sqrt(b * b + AMSD_L) + AMD_L)
        Ma = 0.2 * (a * math.log(a + math.sqrt(a * a + AMSD_b))- a * log_GMD_b - math.sqrt(a * a + AMSD_b) + AMD_b)
        Mb = 0.2 * (b * math.log(b + math.sqrt(b * b + AMSD_a))- b * log_GMD_a - math.sqrt(b * b + AMSD_a) + AMD_a)
        L = 2 * N * N * (La + Lb - (Ma + Mb))
        return L
    except:
...
def findFerriteRodN(I : float, Lr : float, Dr : float, mu : float, dc : float, s : float, dw : float, p : float, result : _CoilResult):
    ...
    kx = -440.9943706 * math.pow(x2, 8) + 1318.707293 * math.pow(x2, 7) - 1604.5491034 * math.pow(x2, 6) + 1021.078226 * math.pow(x2, 5) - 363.8218957 * math.pow(x2, 4) + 71.6178135 * math.pow(x2, 3) - 7.6027344 * math.pow(x2, 2) + 0.3013663 * x2 + 0.995
    if ((kx < 0) or (kx > 1)):
        return
    ...
    while (iTmp < I):
        ...
        if (mu > 1):
            Canf = 5e-4 * M_PI * e0 * (Lr - lc) / (math.log(2 * (Lr + Dr) / Dr) - 1)
...
def findSheildedInductance(I : float, D : float, Ds : float, l : float, Hs : float) -> float:
  result= I * (1 - (D / Ds) * (D / Ds) * (D / Ds)) * (1 - (l / (2 * Hs)) * (l / (2 * Hs)))
  return result
...
def findRMCore_I (N : float, a : float, b : float, c : float, e : float, d2 : float, d3 : float, d4 : float, h1 : float,  h2, g : float, mu : float, type : int, result : _CoilResult) -> float:
     ...
     p = math.sqrt(2.0) * a - b
     lmin = 0.5 * (d2 - d3)
     lmax = math.sqrt(0.25 * (d2 * d2 + d3 * d3) - 0.5 * d2 * d3 * math.cos(alpha - betta))
     if (type == 2):
         lmax = math.sqrt(0.25 * (d2 * d2 + d3 * d3) - 0.5 * d2 * d3 * math.cos(alpha - betta)) - 0.5 * b / math.sin(0.5 * phi)
     if (type == 3):
         lmax = 0.5 * e + 0.5 * (1 - math.sin(M_PI / 4.0)*(d2 - c))
     f = (lmax + lmin) / (2.0 * lmin)
     A7 = 0.25 * (0.5 * betta * d2 * d2 + 0.5 * e * e * math.tan(betta) - 0.5 * e * e * math.tan(alpha - 0.5 * phi)-0.25 * M_PI * d3 * d3)
     if (type == 1):
         A7 = 0.25 * (0.5 * betta * d2 * d2 + 0.5 * d2 * d3 * math.sin(alpha - betta) + 0.5 * math.pow(c - d3, 2) * math.tan(0.5 * phi) - 0.25 * M_PI * d3 * d3)
     if (type == 2):
         A7 = 0.25 * (0.5 * betta * d2 * d2 - 0.25 * M_PI * d3 * d3 + 0.5 * (b * b - e * e) * math.tan(alpha - 0.5 * phi) + 0.5 * e * e * math.tan(betta))
     if (type == 3):
         A7 = 0.25 * (0.5 * betta * d2 * d2 - 0.5 * phi * d3 * d3 + 0.5 * c * c * math.tan(alpha - betta))
     A8 = (M_PI / 16) * (d2 * d2 - d3 * d3)
     D = A7 / A8

     l1 = h2
     A1 = 0.5 * a * a * (1 + math.tan(betta - M_PI / 4.0)) - 0.5 * betta * d2 * d2 - 0.5 * p * p
     sum11 = l1 / A1
     sum12 = l1 / (A1 * A1)
...
def findBrooksCoil(I : float, d : float, pa : float, pr : float):
    ...
    if (nLayer > 1):
        for j in range(nLayer - 2 + 1):
            lengthWire += math.sqrt( ha * ha + math.pow( M_PI * (2 * c + d + 2 * hr * j), 2))
